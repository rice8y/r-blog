...
使用例を以下に示す<sup><a href="#f1">1</a></sup>. 

```latex\documentclass{ltjsarticle}
\usepackage{texshade}
\thispagestyle{empty}
\begin{document}
...
---
<p id="f1"><strong>1.</strong> DVI ファイルを経由する LaTeX エンジンを使用している場合は, TeXshade パッケージを読み込む際に, ドライバオプションで DVI ドライバを指定する必要がある.</p>