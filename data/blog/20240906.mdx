...
上記の実行結果より, エンコードされた招待コード `SEdCTUMtOFNDQk8tMUZNODAtOTMwQUI=` が得られた. これを Base64 でデコードする.
...
これにより, 招待コード `HGBMC-8SCBO-1FM80-930AB` が得られた.
...
まず, `/invite` で招待コード `HGBMC-8SCBO-1FM80-930AB` を入力し, `Sign Up` をクリックする.
