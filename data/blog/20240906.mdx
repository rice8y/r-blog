...
└─$ curl -sX POST http://2million.htb/api/v1/invite/generate | jq
{
  ...
  "success": 1,
  "data": {
    "code": "MUFNSlktUjI3SDUtNEEzWFMtTTFLN0s=",
    "format": "encoded"
  }
}
...
上記の実行結果より, エンコードされた招待コード `MUFNSlktUjI3SDUtNEEzWFMtTTFLN0s=` が得られた. これを Base64 でデコードする.

```shell
┌──(kali㉿kali)-[~]
└─$ echo "MUFNSlktUjI3SDUtNEEzWFMtTTFLN0s=" | base64 --decode
1AMJY-R27H5-4A3XS-M1K7K
```

これにより, 招待コード `1AMJY-R27H5-4A3XS-M1K7K` が得られた.
...
まず, `/invite` で招待コード `1AMJY-R27H5-4A3XS-M1K7K` を入力し, `Sign Up` をクリックする.
...
`Login` により, `/home` にリダイレクトした. `.ovpn` を入手するために, `Access` をクリックする.
